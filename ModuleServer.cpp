...
namespace ModuleServer
{
    ...
    { // Pages principales
        ...
        { // en AP et STA mode
            server.send(200, "text/html", pages[RMS_PAGE_CONNECT_HTML]);
        }
        else
        { // Station Mode seul
            server.send(200, "text/html", pages[RMS_PAGE_MAIN_HTML]);
        }
    }
    void handleMainJS()
    {                                                               // Code Javascript
    ...
    { // Envoi des dernières données  brutes reçues du RMS
        ...
        const char *sourceName = ModulePowerMeter::getSourceName();

        if (source == ModulePowerMeter::SOURCE_PROXY)
        {
    ...
    { // Données page d'accueil
        ...
        if (Source_data == "UxIx2" || (Source_data == "ShellyEm" && EnphaseSerial.toInt() < 3))
    ...
    void handleParaAjax()
    {
        ...
        S += RS + MQTTPrefix + RS + MQTTdeviceName + RS + String(ModuleCore::getName()) + RS + nomSondeFixe + RS + nomSondeMobile;
    ...
    {
        String S = Source + RS + Source_data + RS + String(ModuleCore::getName()) + RS + VERSION + RS + nomSondeFixe + RS + nomSondeMobile + RS + String(RMSextIP);
        S += RS + nomTemperature;
        server.send(200, "text/html", S);
    }
    void handleSetGpio()
    ...
    void handleAP_SetWifi()
    {
        ...
        ModuleDebug::stockMessage("Wifi Begin : " + ssid);
